###################################################################################################
# Program: script to create multiple lines with GPS track text file
...
# template = r"C:\Users\nwb31\Desktop\PA3\PA14\Data\ShapeFiles\gpsTrack.shp"
# outFC = r"C:\Users\nwb31\Desktop\PA3\PA14\Data\ShapeFiles\MulTrackEvaM.shp"
# gpsFile = r"C:\Users\nwb31\Desktop\PA3\PA14\Data\TextFiles\track.txt"
# coordsys = r"C:\Users\nwb31\Desktop\PA3\PA14\Data\ShapeFiles\WGS 1984 UTM Zone 15N.prj"
...
nextTrackIndex = lstValue.index("new_seg")  # retrieve index value of y coordinate in the list
...
lid = 0                                     # counting variable for feature ID
...
with arcpy.da.InsertCursor(featClass, ["ID", "Shape@"]) as isCursor:
    for line in lines:                           # loop through each line of the gps text file
        ...
        isNew = segment[nextTrackIndex].upper()  # retrieve value stored at the index position and convert to uppercase

        if isNew == "TRUE":                             # if point is a starting for a new line
            if lineArray.count > 0:                     # if existing array has already created a line
                newLine = arcpy.Polyline(lineArray)     # create new line
                isCursor.insertRow([lid, newLine])      # add new line feature into the feature class

                lineArray.removeAll()                   # empty the current array to prepare for the next line
            ...
            lid = lid + 1                                   # increment counting variable
            arcpy.AddMessage("Insert track line {0}.".format(lid))     # print confirmation message
            # print("Insert track line {0}.".format(lid))     # print confirmation message
